...
const MyModal = ({ isOpen, closeModal, uniqueKey, pk, name, description, price, isManager, dietaryRequirement, isPopular, minutes}: MyModalProps) => {
  ...
  const translateDR = (dietaryRequirement: "DF" | "GF" | "V" | "VG" | "") => {
    ...
    if (dietaryRequirement === 'GF') return 'Gluten Frees';
    if (dietaryRequirement === 'VG') return 'Vegan';
    if (dietaryRequirement === 'V') return 'Vegetarians';
        ...
        <Dialog as="div" className="relative z-10" onClose={closeModal}>
          ...
          <div className="fixed inset-0 overflow-y-auto no-select">
            <div className="flex min-h-screen items-center justify-center">
              ...
              >
              <div className={`bg-white flex overflow-hidden shadow-xl transition-all h-[500px] rounded-sm w-[500px] ${modalHorizontal ? 'w-[960px]' : 'w-[500px]'}`}>
                <Dialog.Panel className={`w-full flex h-auto ${modalHorizontal ? 'h-full' : 'flex-col overflow-y-auto'} bg-white`}>

                  {/* image */}
                  <section className={`relative w-1/2 h-full bg-red-200 hover:cursor-pointer hover:bg-slate-100`} onClick={() => closeModal()}>
                    ...
                    <section className={`${isPopular ? 'items-start' : '' } p-7 flex flex-col h-[430px]`}>
                      ...
                      {editMode &&
                        <div className="w-full flex items-center justify-center mt-3">
                          <button 
                            className={`${!menuPopular ? 'bg-red-600 hover:bg-red-400' : 'bg-green-600 hover:bg-green-400' } text-white text-[14px] font-bold p-2 rounded-lg `}
                            onClick={() => {
